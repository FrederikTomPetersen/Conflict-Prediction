...
#Random forest prediction death in teh following month 
...
model <- randomForest(deaths ~ ., data=train_data ,ntree=250, importance=TRUE)
model
setwd(Models)
save(model, file = "2_rf_death.rda")
...
#Predictions
test_data$deaths_pred = predict(model, newdata = test_data)
test_data$pred_res = test_data$deaths - test_data$cwm_pred

mean(test_data$pred_res)
sd(test_data$pred_res)

ggplot(data=test_data) +
  geom_point(aes(x = deaths, y = deaths_pred, color =test_data$pred_res )) +
